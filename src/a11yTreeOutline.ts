...
export class A11yTreeOutlineProvider implements vscode.TreeDataProvider<a11yNode> {
	private _onDidChangeTreeData: vscode.EventEmitter<a11yNode | null> = new vscode.EventEmitter<a11yNode | null>();
	readonly onDidChangeTreeData: vscode.Event<a11yNode | null> = this._onDidChangeTreeData
		.event;

	private tree: a11yNode[];
	private text: string;
	private editor: vscode.TextEditor;
	private filterType = GuideDogFilter.OnlyTabableElements;
	...
	private onActiveEditorChanged(): void {
		if (vscode.window.activeTextEditor) {
			if (vscode.window.activeTextEditor.document.uri.scheme === 'file') {
				const enabled = vscode.window.activeTextEditor.document.languageId === 'html';

				vscode.commands.executeCommand('setContext', 'a11yTreeEnabled', enabled);
